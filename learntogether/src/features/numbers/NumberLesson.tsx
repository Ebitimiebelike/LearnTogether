...
export function NumberLesson({ item }: NumberLessonProps) {
  ...
  const say = () => audio.speakNumber(item.writtenWord ?? item.value, item.audio);
    ...
    <>
      ...
      <div className="flex flex-col gap-5 px-5 pb-8">
        <button
          ...
          className="flex flex-col items-center gap-1 rounded-card bg-surface py-6 shadow-card transition-transform duration-150 active:scale-[0.98]"
        >
          <span className="text-[clamp(6rem,34vw,11rem)] font-extrabold leading-none">
            {item.displayValue}
          </span>
          <span className="text-4xl font-bold text-ink-muted">{item.writtenWord}</span>
        </button>

        <QuantityDisplay item={item} />

        <AudioButton onPlay={say} label={`Say ${item.writtenWord}`} className="w-full">
        ...
        <div className="flex gap-3">
          ...
          <Button
            ...
            className="flex-[2]"
            onClick={() =>
              router.push(next ? `/learn/numbers/${next.value}` : "/learn/numbers")
            }
          >
