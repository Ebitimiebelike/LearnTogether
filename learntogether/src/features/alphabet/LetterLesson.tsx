    ...
    <>
      ...
      <div className="flex flex-col gap-5 px-5 pb-8">
        ...
        <button
          ...
          className="flex flex-col items-center rounded-card bg-surface py-6 shadow-card transition-transform duration-150 active:scale-[0.98]"
        >
          <span className="text-[clamp(6rem,34vw,11rem)] font-extrabold leading-none">
            {item.value}
          </span>
          <span className="text-[clamp(3rem,18vw,6rem)] font-bold leading-none text-ink-muted">
            {item.lowercase}
          </span>
        </button>
        ...
        <div className="flex gap-3">
          {previous && (
            <Button
              ...
              icon="back"
              className="flex-1"
              onClick={() => router.push(`/learn/alphabet/${previous.lowercase}`)}
            >
              {previous.value}
            </Button>
          )}
          <Button
            ...
            className="flex-[2]"
            onClick={() =>
              router.push(next ? `/learn/alphabet/${next.lowercase}` : "/learn/alphabet")
            }
          >
